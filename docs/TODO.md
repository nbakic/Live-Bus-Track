...
| C8 | Internet vs. poslužitelj na ekranima koji dohvaćaju podatke | `ui/common/LoadError.kt`, `EmptyState.kt`, ekrani | **DJELOMIČNO (2026-05-29).** Razlikovanje "nema interneta" vs "poslužitelj nedostupan" izvedeno na karti (`LiveStatusPill`) i `RoutesScreen` (+ retry), uz zajednički `EmptyState`/`LoadErrorState`. **Preostaje** proširiti na ekrane koji *dohvaćaju* podatke: `AlertsScreen`, `StopDetailScreen` (dolasci) i `RoutinesScreen` (živi dolasci) — RT degradacija; te `NearbyScreen` (kad je lokalna baza prazna, isti slučaj kao Linije). **Ne dirati** `FavoritesScreen` i `SearchScreen` — njihova prazna stanja su legitimna ("nema omiljenih", "nema rezultata"), nisu greške. |
